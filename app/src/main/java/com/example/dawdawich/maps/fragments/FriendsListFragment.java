...
public class FriendsListFragment extends Fragment implements AdapterView.OnItemClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        FriendsListFragment.inflater = inflater;
        FriendsListFragment.container = container;

        if (adapter == null)
        {
            if (UserController.getInstance(getContext()).getUser().getFriends().size() == 0) {
                List<User> userList = new ArrayList<>();
                userList.addAll(UserController.getInstance(getContext()).getUser().getFriends());
